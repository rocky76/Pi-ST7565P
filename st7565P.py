...
class Glcd(object):
    """ST7565P graphics lcd module object
    ...
    def init(self):
        ...
        self.send_command([self.CMD_SET_RESISTOR_RATIO | 0x4])
        # Turn on display
    ...
    def flip(self):
        """Send back buffer to ST7565P display"""
    ...
    def clear_status_icons(self):
        ...
        # Move to zero position on specified page
        # Set page
        self.send_command([self.CMD_SET_PAGE | 8])
        # Set lower bits of column
        self.send_command([self.CMD_SET_COLUMN_LOWER | (0 & 0xf)])
        # Set upper bits of column
        self.send_command([self.CMD_SET_COLUMN_UPPER | ((0 >> 4) & 0xf)])

        #self.move_cursor(0, 8)
        # Send list of zeros to clear page
        self.send_data([0] * self.LCD_WIDTH)

    def set_status_icon(self, column, state):
        """!!!EXPERIMENTAL!!! Control a status icon on the CFAG12864U4-NFI LCD.
        Args:
            column (int): The column address of the icon (0-127).
            state (bool): True to turn the icon ON, False to turn it OFF.
        """
        # Select Page 8 (icon control area)
        self.send_command([0xB8])  # Page Address = 8

        # Set Column Address to target icon's position
        self.send_command([0x10 | (column >> 4)])  # Set higher nibble (MSB)
        self.send_command([0x00 | (column & 0x0F)])  # Set lower nibble (LSB)

        # Write data (0x01 to turn ON, 0x00 to turn OFF)
        self.send_data([0x01] if state else [0x00])